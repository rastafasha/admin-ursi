...
export class ExpocafEditComponent implements OnInit {
  ...
  public afuConfig = {
    ...
    maxSize: '2',
    uploadAPI: {
      url: environment.apiUrl + '/expocaf/upload',
      headers: {
  ...
  deleteFotoPerfil(){
    this.expocafService.deleteFoto(this.cursoForm.value['id']).subscribe(response => {
  ...
  editCurso(){
    ...
    if(id){
      ...
      this.expocafService.updateExpocaf(data, +id).subscribe(
        resp =>{
    ...
    const data = {
      ...
      this.expocafService.createExpocaf(data).subscribe(
        (resp: any) =>{
          this.escuelaSeleccionado = resp;
